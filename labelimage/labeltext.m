...
[barHeight, barWidth, right, bot] = deal(barInfo.barHeight, barInfo.barWidth, barInfo.right, barInfo.bot);
...
for iDim5 = 1:size(img, 5)
    for iDim4 = 1:size(img, 4)
        for iDim3 = 1:size(img, 3)
            RGB = insertText(img(:,:,iDim3, iDim4, iDim5),position,text,'FontSize',28,'BoxOpacity', 0, 'TextColor','white', 'AnchorPoint',"CenterBottom");
